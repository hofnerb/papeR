...
\texttt{papeR}. For a comprehensive overview of the features of \texttt{papeR}
see also the vignette on using \texttt{papeR} to produce Markdown output (e.g.
via \texttt{vignette("using\_papeR", package = "papeR")}). See also there

...
reproducible research. By relying on powerful tools such as the `Sweave` command kit,
or the packages **knitr** and **xtable**, the package can be easily integrated in
existing workflows. 
...
for easy and pretty conversion. Note that we additionally set `cumulative = TRUE` 
to show cumulative frequencies for `Sex`. In `Sweave` we can use 
```r
<<echo = FALSE, results = tex>>=
xtable(summarize(Orthodont, type = "numeric"))
xtable(summarize(Orthodont, type = "factor", variables = "Sex", cumulative = TRUE))
...
thus `\usepackage{booktabs}` is needed in the header of the LaTeX report.
